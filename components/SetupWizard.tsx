    ...
    const poll = async () => {
      try {
        ...
        if (typeof data?.wifi_last_error === "string" && data.wifi_last_error.trim()) {
          ...
          setWifiStatusHint({
            type: "success",
            message: `Connecting to ${targetSsid} and waiting for a DHCP address. After your phone reconnects to the same WiFi, this page will show the direct access link.`,
          });
