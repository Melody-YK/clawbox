...
export interface SetupStatus {
  ...
  wifi_ready_url: string | null;
  wifi_ipv4: string | null;
  wifi_ipv4_url: string | null;
...
export async function getSetupStatus(): Promise<SetupStatus> {
  ...
  if (runtimeWifiConfigured && (!storedWifiConfigured || !!config.wifi_connecting || wifiLastError)) {
    await setMany({
      ...
      wifi_access_url: wifiReadyUrl ?? undefined,
      wifi_ipv4: wifiIpv4 ?? undefined,
    }).catch(() => {});
  ...
  return {
    ...
    wifi_ready_url: wifiReadyUrl,
    wifi_ipv4: wifiIpv4,
    wifi_ipv4_url: wifiIpv4Url,
