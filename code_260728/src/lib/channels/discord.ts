...
export async function getDiscordConfig(): Promise<DiscordConfigView> {
  ...
  const configured = hasToken || Boolean(readString(channel.serverId)) || Boolean(readString(channel.userId));
  return {
    configured,
...
export async function getDiscordStatus(): Promise<DiscordChannelStatus> {
