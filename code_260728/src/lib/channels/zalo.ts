...
export interface ZaloConfigView {
  configured: boolean; enabled: boolean; hasToken: boolean; dmPolicy: string; groupPolicy: string;
}
...
export class ZaloChannelError extends Error {
  constructor(public readonly code: "invalid_token" | "unreachable" | "gateway", message: string) {
    super(message);
...
export async function validateZaloBotToken(value: string, fetcher: typeof fetch = fetch): Promise<ZaloBotIdentity> {
  const token = normalizeZaloBotToken(value);
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
  try {
    const response = await fetcher(`${ZALO_API_ROOT}/bot${token}/getMe`, { method: "POST", headers: { "Content-Type": "application/json" }, cache: "no-store", redirect: "error", signal: controller.signal });
...
export async function saveZaloConfig(input: { botToken?: string; enabled: boolean }): Promise<ZaloConfigView> {
...
export async function getZaloStatus(runner?: CommandRunner): Promise<ZaloChannelStatus> {
  ...
  try { bot = await validateZaloBotToken((await getZaloBotToken()) || ""); } catch { /* runtime status remains authoritative */ }
