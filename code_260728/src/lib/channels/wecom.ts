...
export class WeComChannelError extends Error {
  constructor(public readonly code: "gateway_unavailable", message: string) {
    super(message);
    this.name = "WeComChannelError";
  }
}
...
export async function probeWeComChannel(): Promise<WeComChannelStatus> {
  ...
  let payload: unknown;
  try {
    payload = JSON.parse(await getChannelStatusJson());
  } catch {
    throw new WeComChannelError(
      "gateway_unavailable",
      "OpenClaw returned invalid JSON while checking WeCom status.",
    );
  }
