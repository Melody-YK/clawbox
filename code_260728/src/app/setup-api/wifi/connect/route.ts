...
async function runWifiSwitch(
  ...
  try {
    ...
    await setMany({
      ...
      wifi_access_url: result.status.ipv4 ? `http://${result.status.ipv4}/` : result.status.accessUrl,
      wifi_ipv4: result.status.ipv4 ?? undefined,
    });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      mdnsHost: accessInfo.mdnsHost,
      nextUrlHint: accessInfo.accessUrl,
      message: `The device is switching to ${ssidTrimmed} and waiting for a DHCP address. Reconnect your phone to the same WiFi and keep this page open: once DHCP is ready, you will get an “Open Device” jump button with the full reachable address (including IPv4). If the connection fails, reconnect to the setup hotspot and try again.`,
    });
  } catch (err) {
    await setMany({
      wifi_configured: false,
