...
interface ChannelSetupExtrasProps {
  ...
  onCompletionChange: (channel: AdditionalChannelId, complete: boolean) => void;
}

export type AdditionalChannelId =
  | "discord"
  | "zalo"
  | "zalo-clawbot"
  | "zalouser"
...
function statusText(status: StatusResponse | null, t: Translate): string {
  if (!status) return t("Not checked yet");
  if (status.connected === true || status.state === "connected") return t("Connected");
  if (status.state === "error") return status.lastError || t("Connection check failed");
...
function isConnected(status: StatusResponse | null): boolean {
...
export default function ChannelSetupExtras({ canConfigure, activeChannel, onCompletionChange }: ChannelSetupExtrasProps) {
  ...
  const loadClawbot = useCallback(async () => {
    try {
  ...
  const loadSignal = useCallback(async () => {
    try {
      const data = await requestJson<StatusResponse>("/setup-api/channels/signal/status", { method: "GET" });
  ...
  useEffect(() => {
    if (personalQr.session?.state === "connected") void loadPersonal();
  }, [loadPersonal, personalQr.session?.state]);

  useEffect(() => {
    if (signalQr.session?.state === "connected") void loadSignal();
  }, [loadSignal, signalQr.session?.state]);

  useEffect(() => {
    if (!discordStatus) return;
    onCompletionChange("discord", isConnected(discordStatus));
  ...
  useEffect(() => {
    ...
    onCompletionChange("signal", isConnected(signalStatus));
  }, [onCompletionChange, signalStatus]);
  ...
  return (
    <div className="min-w-0 space-y-3">
      <ChannelSection id="discord" title="Discord" description={t("Discord uses a Bot Token and Developer Mode IDs; it does not use a QR login")} done={discordStatus?.configured === true} open={activeChannel === "discord"} onToggle={() => {}} t={t}>
      ...
      <ChannelSection id="zalo-clawbot" title="Zalo ClawBot" description={t("Official Zalo Mini App QR login. It creates an owner-bound bot and does not require a developer token")} done={clawbotConfig?.configured === true} open={activeChannel === "zalo-clawbot"} onToggle={() => {}} t={t}>
        {guide(t("How Zalo ClawBot QR login works"), locale === "zh-CN" ? [
          ...
          <>This uses the pinned catalog plugin <code>@zalo-platforms/openclaw-zaloclawbot@0.1.4</code> documented by OpenClaw; the first click installs it from npm if needed.</>,
      ...
      <ChannelSection id="signal" title="Signal" description={t("Signal uses the external signal-cli daemon. QR login links an existing Signal device")} done={signalStatus?.configured === true} open={activeChannel === "signal"} onToggle={() => {}} t={t}>
        ...
        <div className="space-y-3">
          ...
          <p className="break-words text-xs text-[var(--text-muted)]">{statusText(signalStatus, t)}</p>
        </div>
      </ChannelSection>
    </div>
  );
