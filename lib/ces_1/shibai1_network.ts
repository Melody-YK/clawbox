...
const IW_SCAN_TIMEOUT_MS = Math.max(
  5_000,
  Number(process.env.IW_SCAN_TIMEOUT_MS) || 15_000,
);
...
function decodeIwSsid(escaped: string): string {
  // Step 1: Replace \xHH with the actual byte character (Latin-1)
  const latin1String = escaped.replace(
    /\\x([0-9a-fA-F]{2})/g,
    (_, hex) => String.fromCharCode(parseInt(hex, 16))
  );
  
  // Step 2: Convert Latin-1 bytes to UTF-8 string
  // Buffer.from(latin1String, 'latin1') treats each char as a byte
...
function parseIwScan(stdout: string): WifiNetwork[] {
  ...
  for (const block of blocks) {
    ...
    // Extract security (simplified)
    const hasWpa = block.includes('RSN:') || block.includes('WPA:');
    const hasWep = block.includes('WEP');
    const security = hasWpa ? 'WPA2' : (hasWep ? 'WEP' : 'none');
...
async function isAPMode(iface?: string): Promise<boolean> {
  ...
  try {
    const { stdout } = await exec("sudo", ["iw","dev", target, "info"], { timeout: NETWORK_TIMEOUT });
...
//Decode SSID from iw scan output.k
...
//  }
    ...
    // ========== NEW: Try iw scan first if in AP mode ==========
  if (wasAP) {
    const iwNetworks = await iwScan(iface);
    if (iwNetworks !== null && iwNetworks.length > 0) {
      console.log(`[WiFi] iw scan succeeded in AP mode, found ${iwNetworks.length} networks`);
      return iwNetworks;
    }
    console.warn("[WiFi] iw scan unavailable or empty, falling back to nmcli teardown");
    
    // Fallback: teardown AP and use nmcli
    await exec("nmcli", ["connection", "down", "ClawBox-Setup"], { timeout: NETWORK_TIMEOUT }).catch(() => {});
    await new Promise((resolve) => setTimeout(resolve, 1000));
  }
